...
class VGGTransformerEncoderDecoder(nn.Module):
    ...
    def forward(self, src, tgt, wav_len, pad_idx=0):
        print(f"🔹 [STEP 1] Input src.shape: {src.shape}")  # (B, T, C, Feat)
        print(f"🔹 [STEP 1] Input tgt.shape: {tgt.shape}")  # (B, T)
        print(f"🔹 [STEP 1] Input wav_len.shape: {wav_len.shape}")  # (B,)
        ...
        #     print(f"🔹 [STEP 2] After transpose src.shape: {src.shape}")
        
        for layer_idx, layer in enumerate(self.conv_layers_dec_enc):
            src = layer(src)
            print(f"🔹 [STEP 3] After VGGBlock {layer_idx} (Encoder), src.shape: {src.shape}")

        src = src.permute(0, 2, 1, 3).contiguous()
        print(f"🔹 [STEP 4] After permute src.shape: {src.shape}")

        bsz, time, c, feat = src.shape
        src = src.view(bsz, time, c * feat)
        print(f"🔹 [STEP 5] Final src.shape before Transformer: {src.shape}")

        abs_len = torch.clamp(torch.round(wav_len * time).long(), max=time)
        print(f"🔹 [STEP 6] abs_len: {abs_len}")
        ...
        print(f"🔹 [STEP 7] After custom_src_module src.shape: {src.shape}")

        encoder_out, _ = self.encoder(
            src=src, src_mask=src_mask, src_key_padding_mask=src_key_padding_mask, pos_embs=None
        )

        print(f"🔹 [STEP 8] Transformer Encoder output.shape: {encoder_out.shape}")

        tgt = self.custom_tgt_module(tgt)
        print(f"🔹 [STEP 9] After tgt embedding tgt.shape: {tgt.shape}") # B x T x C 
...
# ====== Dữ liệu giả lập mới ======
batch_size = 4
time_steps = 2050  # Số khung thời gian tối đa
features = 80  # Số feature mỗi frame

# Tạo src ngẫu nhiên với shape (B, T, F)
src = torch.randn(batch_size, time_steps, features).to(device)

# Tạo tgt ngẫu nhiên (B, T) với vocab size 1000
tgt = torch.randint(0, tgt_vocab, (batch_size, 50)).to(device)

# Tính wav_len từ src
wav_len = compute_wav_len_from_src(src.unsqueeze(2), threshold=0.1)  # Cần thêm dim C=1 tạm thời để tính năng lượng
print(f"✅ Generated wav_len: {wav_len}")  # Debug giá trị wav_len
...
# ====== Chạy thử DECODER ======
print("\n🔹 [TEST] Running Decoder...")

# Giả lập đầu vào cho decoder với câu có 10 từ
